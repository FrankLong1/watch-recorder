...
type WatcherStore = Pick<MemoStore, "getWatcherMemo" | "listWatcherMemosAfter">;
type WatcherLog = (message: string, fields?: Record<string, string | number>) => void;
type WatcherAuthorization = (
...
export function registerWatcherFeed(
  ...
  app.get("/v1/watcher/memos", async (c) => {
    ...
    try {
      const memos = await options.store.listWatcherMemosAfter(authorization.userId, cursor, limit);
      return c.json({ memos });
    } catch (error) {
  ...
  app.get("/v1/watcher/memos/:id", async (c) => {
    const authorization = await options.authorize(c.req.header("authorization"));
    if (authorization === "unauthorized") return c.json({ error: "unauthorized" }, 401);
    if (authorization === "forbidden") return c.json({ error: "forbidden" }, 403);
    const id = c.req.param("id");
    if (!UUID.test(id)) return c.json({ error: "id must be a uuid" }, 400);

    try {
      const memo = await options.store.getWatcherMemo(authorization.userId, id);
      return memo ? c.json({ memo }) : c.json({ error: "not found" }, 404);
    } catch (error) {
      if (error instanceof DatabaseUnavailableError) {
        options.log("watcher memo database unavailable");
        return c.json({ error: "database unavailable" }, 503);
      }
      options.log("watcher memo failed", { detail: error instanceof Error ? error.name : "unknown" });
      return c.json({ error: "internal error" }, 500);
    }
  });
}
