...
thought occurs
  ...
  → speak normally
  → silence, Double Tap, or second Action press ends it
  → a saved haptic confirms the thought is safe
```
...
| Second Action-button press | Add after device verification | Makes the dominant Ultra path physically symmetric. |
...
The code must distinguish two promises:

1. **Press acknowledged** — the watch saw the action.
2. **Speak now** — the microphone is live and the first sample is reaching the
   capture file.

Those may be separate haptics. A false “speak now” signal is worse than a
slightly slower-feeling interaction because it clips the opening words that
often carry the thought’s meaning.

Measure real-device first-word clipping, not only milliseconds to a red screen.
The full rationale and measurement plan are in `LATENCY.md`.

## Ending without ceremony

The user should normally stop doing nothing. Silence detection can end the memo
only after an arm → warn → veto period: a distinct haptic announces the pending
save, speech or a deliberate movement vetoes it, and a later success haptic
confirms the local commit.

Double Tap and a second Action-button press are faster explicit endings. The
large Stop control remains as the universal, deterministic fallback. It must be
the only meaningful recording-screen target; it is not an invitation to add a
...
`FAILURE_MODES.md` is the operating catalogue for preserving this trust.
