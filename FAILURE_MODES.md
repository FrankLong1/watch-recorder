...
([`MemoStore.finalize`](WatchApp/MemoStore.swift)), and neither leg deletes its
source. Most failures are *stalls*, not losses.

Each device does eventually delete its copy — 24 h after the next hop has taken
it ([`Retention`](Shared/Retention.swift)) — but every failure in this document
...
([1_INGEST_ARCHITECTURE.md](1_INGEST_ARCHITECTURE.md) §"The one-way door")
...
| Mic permission undetermined on first press | Intent foregrounds the app, `startRecording` requests permission ([`RecorderModel.swift:197`](WatchApp/RecorderModel.swift)) — a prompt appears, and **the user is already talking** | 🔴 |
| Mic permission denied | `requestPermission` returns false, failure haptic, no recording | 🔵 (haptic only) |
...
| Watch busy: on a call, in Walkie-Talkie, mid-Siri | Foregrounding competes with an audio-owning app; activation may fail | 🔴 |
| Storage full at press time | `store.newCaptureURL` throws → `phase = .failed`, failure haptic | 🔵 |
...
| Cold-launch latency exceeds the user's patience | The first ~1 s of speech lands before `record()` returns. Pre-arming ([`prearm`](WatchApp/RecordingEngine.swift)) shortens this; it cannot remove it | 🟠 |
...
([`recoverOrphanedCaptures`](WatchApp/MemoStore.swift)).

| Failure | What happens today | Class |
|---|---|---|
| Phone call arrives mid-recording | `AVAudioSession` interruption → pause, auto-resume on `.shouldResume` ([`handleInterruption`](WatchApp/RecorderModel.swift)) — the memo has a gap but stays one file | 🟠 |
...
| Battery reaches 5% | Polled every ~5 s, auto-saves ([`checkBattery`](WatchApp/RecorderModel.swift)) | 🔵 |
...
| AAC compression fails at save | Raw `.caf` is kept instead — "a large memo beats a lost one" ([`finalize`](WatchApp/MemoStore.swift)). The phone atomically normalises it to M4A before ingest; if that fails, the CAF remains locally pending rather than being mislabelled | 🟡 |
...
| Failure | What happens today | Class |
|---|---|---|
| Phone left at home / out of Bluetooth range and off WiFi | Transfer queues on the watch indefinitely. This is the first red box in [1a_UPLOAD_PATHS.md](1a_UPLOAD_PATHS.md) | 🟡→🔴 |
...
| Bad `OPENAI_MODEL` | Boots clean, fails on the first real memo as `502` — named in [server/README.md](server/README.md) | 🔴 |
...
## Stage 5 — transcript → todos → execution (unbuilt)

[2_AGENT_ARCHITECTURE.md](2_AGENT_ARCHITECTURE.md) is a proposal, so these are
design risks rather than live behaviour.

| Failure | Consequence |
|---|---|
| Polling script not running | Transcripts accumulate; nothing ever notices |
| Dev server (a Mac) asleep, closed, or on a different network | The entire stage stalls, indefinitely and silently |
| Agent crashes mid-memo after claiming it | The row stays claimed forever unless claims expire |
| Two agents merge `todo.json` concurrently | Lost writes — already mitigated by per-memo files, but the merge step is the new race |
| Agent hallucinates a todo from an ambiguous memo | A task appears that the user never asked for |
| **Execution agent acts on a mis-transcribed memo** | The blast radius of stage 4's 🟠 class stops being "a wrong note" and becomes "an action taken in the world" |
| Judge agent tags something executable that isn't | Same, one step earlier |
| No dead-letter anywhere | A memo that fails every stage looks exactly like a memo that was never spoken |

The compounding risk is worth stating plainly: **stage 5 turns silent-wrong into
silent-wrong-and-acted-upon.** Every mitigation for stage 4's accuracy problems
gets more valuable the moment an execution agent exists.
...
trade from [1_INGEST_ARCHITECTURE.md](1_INGEST_ARCHITECTURE.md), but it is also
