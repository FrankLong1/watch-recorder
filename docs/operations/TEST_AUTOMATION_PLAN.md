...
1. `scripts/sim.sh`: a shell smoke harness that builds, installs, drives the existing
...
- `src/swift_app/Shared/Capture/RecordingLaunchRequest.swift` consumes `WristMemoAutoRecord` in
  DEBUG builds.
- `src/swift_app/WatchApp/Capture/RecorderModel.swift` consumes `WristMemoAutoStopAfter` and already
  saves after that duration.
- `src/swift_app/WatchApp/Storage/MemoStore.swift` uses a stable data layout under the application
...
`docs/operations/DEVICE_TESTING.md` still exists. Parse `simctl list devices --json` with the
...
workflow in `docs/operations/DEVICE_TESTING.md` makes the first five expected, while item six
...
Create `src/swift_app/Tests/Unit/` and a `WristMemoTests` watchOS unit-test bundle.
...
- `src/swift_app/Shared/Support/Formatting.swift`
- `src/swift_app/Shared/Capture/RecordingLaunchRequest.swift`
- `src/swift_app/Shared/Support/SharedConfig.swift`
- `src/swift_app/Shared/Media/AudioDuration.swift`
- `src/swift_app/WatchApp/Storage/Memo.swift`
- `src/swift_app/WatchApp/Storage/MemoStore.swift`
- `src/swift_app/Shared/Media/AudioCompressor.swift`
...
```bash
xcodebuild -project src/swift_app/WristMemo.xcodeproj -list
xcodebuild -project src/swift_app/WristMemo.xcodeproj -scheme "WristMemo Watch App" \
  -destination "platform=watchOS Simulator,id=$SIM" test
```

Also run `./scripts/sim.sh --scenario all` with a live simulator. The final CI/local
...
2. Add `scripts/sim.sh` lifecycle helpers and make `record-save` green.
...
6. Wire both test bundles into the shared scheme, run the verification commands,
   and update `docs/operations/DEVICE_TESTING.md` to point to `scripts/sim.sh` as the repeatable path.
