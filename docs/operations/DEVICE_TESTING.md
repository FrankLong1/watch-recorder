...
> **Most changes do not need hardware.** `./scripts/sim.sh` runs the unit bundle, taps
...
```
open /Users/frank/Projects/watch-recorder/src/swift_app/WristMemo.xcodeproj
```
...
Or from the command line. The phone route is the reliable default because the
paired iPhone delivers the embedded Watch app without the fragile Mac → Watch
Wi-Fi tunnel:

```bash
xcodebuild -project src/swift_app/WristMemo.xcodeproj -scheme "WristMemo" \
...
`./scripts/run.sh` automates that path. Use `./scripts/run.sh --watch` only
when the direct Watch tunnel is healthy.
...
- [ ] **A second Action press does not stop the recording.** The Action Button
      is start-only; Double Tap, the screen tap, and wrist-down inactivity are
      the explicit stop paths.
...
- [ ] The **START** haptic is unmistakable and fires only when the microphone is
      actually writing audio.
- [ ] The **STOP** haptic is unmistakable and fires for every end path.
- [ ] Red is bearable at night, and the grey reads as off rather than as an
      unlit screen, in sunlight and in always-on
- [ ] Double Tap stops a recording and does nothing while READY
- [ ] Lowering the wrist stops a recording after the 8-second inactivity timeout
- [ ] Pressing the Digital Crown to exit WristMemo stops and saves immediately
...
- **The Action button is Ultra-only.** On a Series 9 or any non-Ultra watch,
  ...
  `supportedModes = .foreground(.immediate)`. Adding the **Your Agents**
