...
/// The entire app.
///
/// One control, the size of the screen. Grey means ready, red means the
/// microphone is open, and a tap anywhere is the only gesture there is. No
/// timer, no meter, no cancel, no list, no settings, no navigation: everything
/// a memo needs after it is spoken — compress, sync, upload, delete itself —
/// happens without the user, so there is nothing to show and nothing to decide.
///
...
struct RecordScreen: View {
    ...
    var body: some View {
        ...
        // Double Tap cannot launch the app, but once the app is frontmost it
        // drives the same control the screen does — which is the point of there
        // only being one.
        ...
        .accessibilityHint(model.isRecording ? "Stops and saves the memo" : "Starts a memo")
    }
