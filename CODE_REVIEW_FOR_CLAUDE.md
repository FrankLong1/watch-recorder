...
### [P1] Preserve the existing phone copy until WatchConnectivity import succeeds — `iOSApp/PhoneLibrary.swift:176`
...
### [P1] Make raw-Capture fallback uploads use a format the transcription endpoint can read — `iOSApp/TranscriptionClient.swift:106`
...
`server/src/transcribe.ts:49`. A raw PCM CAF fallback is therefore presented as
...
### [P2] Pass the documented deployment values into Terraform — `server/README.md:118`
...
### [P2] Return a terminal client status for non-retryable transcription failures — `server/src/index.ts:140`
...
### [P2] Recreate the background URLSession when iOS wakes the app for it — `iOSApp/WristMemoApp.swift:40`
...
### [P2] Reference the concept image once instead of embedding a duplicate copy — `custom-watch-ultra-bands/index.html:23`
